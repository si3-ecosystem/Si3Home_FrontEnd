import {
  ...
  SelectField,
  TextAreaField,
} from "@/app/(base)/onboard/components/Input";
  ...
  return (
    <div className="flex gap-2">
      <p className="text-xs sm:text-base">
        {props.title ?? "Prefer to describe"}:{" "}
      </p>
      <div className="flex-1">
        <input
          type="text"
          onChange={(e) => props.setValue!(e.target.value)}
          className="sm:w-10/12 outline-0  border-b-2 border-black bg-transparent"
        />
      </div>
    </div>
  );
...
export function SelfIdentityField(props: FormFieldProps) {
  ...
  return (
    <div className="my-4">
      <p className="max-w-[444px] text-sm sm:text-lg">
        ...
        {identities.map((identity) => {
          if (identity == props.value) {
            return (
              <button
                type="button"
                key={identity}
                className="rounded-[21px] px-3 py-[10px] bg-black text-white font-medium text-sm"
              >
                {identity}
              </button>
            );
          ...
          return (
            <button
              ...
              key={identity}
              className="rounded-[21px] px-3 py-[10px] bg-[#e9e9e9] font-medium text-sm"
            >
              {identity}
            </button>
          );
      ...
      {preferToDescribeShown && (
        <PreferToDescribeForm setValue={props.setValue} />
      )}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="my-8">
        <SelectField
          ...
          options={[]}
          label={
            <p className="text-sm sm:text-lg capitalize">Grant provider</p>
          }
        />
      </div>
      <div className="my-8">
